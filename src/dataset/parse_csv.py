...
def dataset_cleanup(dataset):
    ...
    for obj in dataset:
        ...
        if obj['Lanzamiento'] in ['Discontinued', "Cancelled"]:
            ...
            continue
        try:
            obj['Lanzamiento'] = re.search('.*(20\d+).*', obj['Lanzamiento']).group(1)
        ...
        for variant in memory_variants:
            ...
            if len(memory_data) == 0:
                print("Skipping phone variant, missing information for memory", variant, memory_data)
                continue
            memory_fields = ['RAM', 'Almacenamiento Interno', 'Extra memory data']
            memory_data.reverse()
...
with open(in_filename, 'r') as csvfile:
    ...
    print(field_mapping)
    object_dataset = [get_obj_from_row_and_mapping(row, field_mapping) for row in reader]
